...
# scan for application files not listed in .workflow_plan_files.
# If drift is found, write .workflow_drift — this blocks the next pre_tool_use call.
if [[ "$STATE" == "IMPLEMENT" && "$TOOL_NAME" == "Bash" && -x "$CHECK_DRIFT" ]]; then
  # Skip if drift flag already exists (already blocked; don't overwrite the list)
