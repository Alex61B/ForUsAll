...
#
# State is written to .workflow_state.
# Failure count is written to .workflow_failures.
# Verification lock is written to .workflow_verified by scripts/verify.sh.
...
# RESEARCH→PLAN gate: docs/research.md must exist with all required sections,
# and PROMPTS.md must have at least one real logged prompt.
check_research_gate() {
  ...
  # Require at least one real prompt logged under '## Prompt Log'
...
# PLAN→IMPLEMENT gate: .workflow_plan_files must exist and be non-empty.
check_plan_gate() {
...
# IMPLEMENT→TEST gate:
#   1. Every file in .workflow_plan_files must exist on disk.
#   2. No unplanned files (drift) may exist in tracked directories.
check_implement_gate() {
  check_plan_gate  # plan file must still exist

  # Check all planned files exist on disk
  local missing=()
...
# TEST→done gate: .workflow_verified must exist (written by verify.sh on success).
check_test_gate() {
...
# Clears the verification lock on all transitions (prevents stale pass from a previous cycle).
clear_verified_lock() {
...
case "$CURRENT:$ACTION" in
  ...
  "RESEARCH:next")
    ...
    echo "  Next: run /plan to define architecture and .workflow_plan_files."
    ;;
  ...
  "PLAN:next")
    ...
    echo "  Failure counter reset to 0."
    echo "  Next: run /implement to build the planned files."
    ;;
  ...
  "IMPLEMENT:next")
    ...
    echo "  Failure counter reset to 0."
    echo "  Next: run /test and execute 'bash scripts/verify.sh'."
    ;;
  ...
  "TEST:next")
    ...
    echo "  Confirm PROMPTS.md and docs/api.md are fully up to date."
    ;;

  # ─── Failure: TEST → RESEARCH (remediation loop) ────────────────────────────
  ...
  "IMPLEMENT:drift-to-plan")
    if [[ ! -f "$DRIFT_FLAG" ]]; then
      ...
      echo "  Use 'drift-to-plan' only when .workflow_drift is present." >&2
      exit 1
    fi
    echo ""
  ...
  "IMPLEMENT:drift-to-research")
    if [[ ! -f "$DRIFT_FLAG" ]]; then
      echo "ERROR: No drift detected (.workflow_drift does not exist)." >&2
      echo "  Use 'drift-to-research' only when .workflow_drift is present." >&2
      exit 1
    fi
    echo ""
    echo "Drift summary (unplanned files):"
    ...
    echo "  Reason: requirements or environment assumptions were incorrect."
    echo "  Review docs/research.md, update it, and re-run the planning cycle."
    ;;
  ...
  *)
    ...
    echo "" >&2
    echo "Valid usage:" >&2
    echo "  bash scripts/advance_state.sh next              — advance forward" >&2
    echo "  bash scripts/advance_state.sh fail              — TEST failure → RESEARCH" >&2
    echo "  bash scripts/advance_state.sh drift-to-plan     — drift recovery → PLAN" >&2
    echo "  bash scripts/advance_state.sh drift-to-research — drift recovery → RESEARCH" >&2
    echo "" >&2
    echo "Current state: $CURRENT" >&2
    case "$CURRENT" in
      RESEARCH) echo "  Next: RESEARCH → PLAN (requires docs/research.md + PROMPTS entry)" >&2 ;;
      PLAN)     echo "  Next: PLAN → IMPLEMENT (requires .workflow_plan_files)" >&2 ;;
      IMPLEMENT)echo "  Next: IMPLEMENT → TEST (requires all files exist + drift-free)" >&2 ;;
      TEST)     echo "  Next: TEST → done (requires .workflow_verified from verify.sh)" >&2 ;;
    esac
    exit 1
    ;;
